...
// mocha's `context` is an alias for `describe`
...
const { controllers, privateDidDocs, capabilities, addToLoader, testLoader } =
  mock
...
describe('zcap', () => {
  describe('Sign with capabilityInvocation proof purpose', () => {
    it('should succeed w/key invoker', async () => {
      ...
      expect(signed).toEqual(mock.exampleDocWithInvocation.alpha)
    })
    ...
    it('should succeed w/controller invoker', async () => {
      ...
      expect(signed).toEqual(mock.exampleDocWithInvocation.beta)
    })
    ...
    it('should fail when missing "capability"', async () => {
      ...
      expect(err.message).toBe('"capability" must be a string or object.')
    })
    ...
    it('should fail when missing "capabilityAction"', async () => {
      ...
      expect(err).toBeDefined()
      expect(err.message).toBe('"capabilityAction" must be a string.')
    })
  ...
  describe('Sign with capabilityDelegation proof purpose', () => {
    it('should succeed w/verification method as controller', async () => {
      ...
      expect(delegatedCapability).toEqual(capabilities.delegated.alpha)
    })
    ...
    it('should fail when missing "parentCapability"', async () => {
      let err
      try {
        const doc = clone(mock.exampleDoc)
        await _delegate({
  ...
  context('Capability controller is itself a verification method', () => {
    ...
    it(
      'should fail to verify a capability chain of depth 2 ' +
        'when the expectedRootCapability does not match',
      async () => {
        const delegatedCapability = await _delegate({
          parentCapability: capabilities.root.alpha,
          controller: bob.get('capabilityInvocation', 0).id,
        ...
        expect(error.message).toContain(
          'does not match expected root capability'
        )
      }
    )
  ...
  context('Capability controller uses verification method', () => {
    ...
    it('should fail to invoke a root zcap with no controller', async () => {
      ...
      expect(result.error.name).toBe('VerificationError')
      const [error] = result.error.errors
      expect(error.message).toContain('Capability controller not found')
    })

    it(
      'should verify a invoking root capability w/ separate target when ' +
        'a matching `expectedRootCapability` is given',
      async () => {
        const root = {
          '@context': ZCAP_CONTEXT_URL,
          id: uuid(),
    ...
    it(
      'should verify a root capability w/ separate target when ' +
        'a matching `expectedRootCapability` array is given',
      async () => {
        const root = {
          '@context': ZCAP_CONTEXT_URL,
          id: uuid(),
    ...
    it(
      'should fail to verify a root capability w/ separate target when ' +
        '`expectedRootCapability` is not a URI',
      async () => {
        const root = {
          '@context': ZCAP_CONTEXT_URL,
          id: uuid(),
    ...
    it(
      'should fail to verify a root capability w/ separate target when ' +
        'no `expectedRootCapability` is given',
      async () => {
        const root = {
          '@context': ZCAP_CONTEXT_URL,
          id: uuid(),
    ...
    it('should verify an invoked root capability w/ multiple controllers', async () => {
      const root = {
        '@context': ZCAP_CONTEXT_URL,
        id: uuid(),
    ...
    it('should fail if expectedRootCapability does not match', async () => {
      ...
      expect(result.error.name).toBe('VerificationError')
      const [error] = result.error.errors
      expect(error.message).toContain('does not match expected root capability')
    })
    ...
    it(
      'should verify invoking a capability chain of depth 2 and a ' +
        '"read" expected action',
      async () => {
        // alice delegates to bob
        const delegatedCapability = await _delegate({
          parentCapability: capabilities.root.beta,
    ...
    it(
      'should verify a capability chain of depth 2 and an ' +
        'allowed action on one capability',
      async () => {
    ...
    it(
      'should fail to verify a capability chain of depth 2 when ' +
        'matching "capabilityAction" not found',
      async () => {
    ...
    it(
      'should fail to verify a capability chain of depth 2 when a ' +
        '"capabilityAction" is not in "allowedAction"',
      async () => {
        // alice delegates to bob with `allowedAction: 'write'`
        const delegatedCapability = await _delegate({
          newCapability: {
    ...
    it(
      'should fail to verify a capability chain of depth 2 when ' +
        'required "capabilityAction" is missing',
      async () => {
    ...
    it(
      'should fail to verify a capability chain of depth 2 when an ' +
        'expected "capabilityAction" does not match',
      async () => {
        // alice delegates to bob w/o any action restrictions
        const delegatedCapability = await _delegate({
          parentCapability: capabilities.root.beta,
    ...
    describe('Chain depth of 3', () => {
      ...
      it(
        'should fail to verify chain with embedded root ' + 'zcap in chain',
        async () => {
          ...
          expect(localError.name).toBe('TypeError')
          ...
          expect(result.error.name).toBe('VerificationError')
          const [error] = result.error.errors
          expect(error.message).toContain('it must consist of strings')
        }
      )

      it(
        'should fail to verify chain with non-embedded last ' +
          'delegated zcap',
        async () => {
          // alice delegates to bob
          const bobZcap = await _delegate({
      ...
      it(
        'should fail to verify chain with misreferenced parent ' +
          'zcap in middle of chain',
        async () => {
          ...
          expect(result.error.name).toBe('VerificationError')
          const [error] = result.error.errors
          expect(error.message).toContain('does not match the parent')
        }
      )

      it(
        'should fail to verify chain with misreferenced parent ' +
          'zcap at end of chain',
        async () => {
          // alice delegates to bob
          const bobZcap = await _delegate({
            parentCapability: capabilities.root.beta,
      ...
      it(
        'should fail to verify a chain ' + 'w/invalid allowedAction strings',
        async () => {
      ...
      it(
        'should fail to verify chain w/invalid allowedAction string ' +
          'vs array',
        async () => {
      ...
      it(
        'should fail to verify chain ' + 'w/invalid allowedAction arrays',
        async () => {
      ...
      it(
        'should verify chain when child allowedAction is ' +
          'a valid subset of the parent allowedAction array',
        async () => {
          // alice delegates to bob w/ allowed action restriction array
          const bobZcap = await _delegate({
            newCapability: {
      ...
      it(
        'should verify chain when child allowedAction is ' +
          'a valid subset of the undefined parent allowedAction',
        async () => {
      ...
      it('should verify chain w/inspectCapabilityChain', async () => {
        ...
        const inspectCapabilityChain = async ({
          ...
          checkedChain++
          expect(Array.isArray(capabilityChain)).toBe(true)
          expect(capabilityChain).toHaveLength(3)
          expect(Array.isArray(capabilityChainMeta)).toBe(true)
          expect(capabilityChainMeta).toHaveLength(3)
        ...
        expect(result).toBeDefined()
        expect(result.verified).toBe(true)
        expect(checkedChain).toBe(1)
      })

      it(
        'should fail to verify w/inspectCapabilityChain ' +
          'w/revoked capability',
        async () => {
          // alice delegates to bob
          const bobZcap = await _delegate({
            parentCapability: capabilities.root.beta,
          ...
          const inspectCapabilityChain = async ({
            ...
            return {
              error: new Error(
                `The capability "${capabilityChain[0].id}" ` +
                  'has been revoked.'
              ),
              valid: false
            }
          ...
          expect(result).toBeDefined()
          expect(result.verified).toBe(false)
          expect(result.error.errors[0]).toBeDefined()
          expect(result.error.errors[0].message).toContain('revoked')
          expect(checkedChain).toBe(1)
        }
      )

      it(
        'should fail to verify w/delegated zcap created before ' + 'parent',
        async () => {
          // alice delegates to bob
          const bobZcap = await _delegate({
            parentCapability: capabilities.root.beta,
      ...
      it(
        'should fail to verify w/delegated zcap with no expiration ' + 'date',
        async () => {
          // TTL of 1 day
      ...
      it(
        'should fail to verify chain w/delegated zcap in the ' + 'future',
        async () => {
          // TTL of 1 day
          const ttl = 1000 * 60 * 60 * 24
      ...
      it(
        'should fail to verify chain ' +
          'w/delegated zcap with a TTL that is too long',
        async () => {
          // TTL of 1 day
          const ttl = 1000 * 60 * 60 * 24
      ...
      it(
        'should verify invoking ' +
          'w/TTL and delegation date monotonicity checks',
        async () => {
          // 24 hour TTL
          const ttl = 1000 * 60 * 60 * 24
      ...
      it(
        'should fail invoking ' +
          'w/inspectCapabilityChain and a revoked capability',
        async () => {
          // alice delegates to bob
          const bobZcap = await _delegate({
            parentCapability: capabilities.root.beta,
          ...
          const inspectCapabilityChain = async ({
            ...
            checkedChain++
            expect(capabilityChain).toBeDefined()
            expect(Array.isArray(capabilityChain)).toBe(true)
            expect(capabilityChain).toHaveLength(3)
            _checkCapabilityChain({ capabilityChain })
            _checkCapabilityChainMeta({ capabilityChainMeta })
          ...
          expect(result.verified).toBe(false)
          expect(result.error.errors[0]).toBeDefined()
          expect(result.error.errors[0].message).toContain('has been revoked')
          expect(checkedChain).toBe(1)
        }
      )
    ...
    describe('Expiration date', () => {
      it('CapabilityInvocation throws TypeError on invalid date', async () => {
        let result
        let err
        try {
          result = new CapabilityInvocation({
            expectedTarget: 'urn:uuid:1aaec12f-bcf2-40d8-8192-cc4dde9bca96',
            suite: new Ed25519Signature2020(),
            date: 'invalid date'
          })
        } catch (e) {
        ...
        expect(err.message).toContain('is not a valid date.')
      })

      it('CapabilityDelegation throws TypeError on invalid date', async () => {
        let result
        let err
        try {
      ...
      it('should fail to verify root capability with `expires`', async () => {
      ...
      it('should verify invoking a capability with `expires`', async () => {
        const rootCapability = { ...capabilities.root.beta }
      ...
      it('should fail to delegate a capability with bad `expires` field', async () => {
        const rootCapability = { ...capabilities.root.beta }
      ...
      it('should fail to verify a capability with bad `expires` field', async () => {
        const rootCapability = { ...capabilities.root.beta }
      ...
      it(
        'should verify invoking a capability with `expires` ' +
          'and `date` parameter in the past',
        async () => {
          const rootCapability = { ...capabilities.root.beta }
      ...
      it(
        'should fail invoking a chain depth 2 capability with `expires` ' +
          'and `date` parameter in the past',
        async () => {
          const rootCapability = { ...capabilities.root.beta }
      ...
      it(
        'should fail invoking a chain depth 2 capability with `expires` ' +
          'and `date` parameter in the future',
        async () => {
          const rootCapability = { ...capabilities.root.beta }
      ...
      it(
        'should fail invoking a chain depth 3 capability with `expires` ' +
          'and `date` parameter in the past',
        async () => {
          const rootCapability = { ...capabilities.root.beta }
      ...
      it(
        'should fail invoking a chain depth 3 capability with `expires` ' +
          'and `date` parameter in the chain in the past',
        async () => {
          const rootCapability = { ...capabilities.root.beta }
      ...
      it(
        'should fail invoking a chain depth 3 capability with `expires` ' +
          'and `date` parameter in the chain in the future',
        async () => {
          const rootCapability = { ...capabilities.root.beta }
      ...
      it(
        'should fail invoking a capability with missing `expires` in ' +
          'the second capability delegation',
        async () => {
      ...
      it(
        'should fail invoking a delegated capability before its ' +
          'delegation proof "created" date',
        async () => {
          const rootCapability = { ...capabilities.root.beta }
      ...
      it(
        'should fail invoking a capability with ' +
          'capability that expires on the Unix epoch',
        async () => {
          const rootCapability = { ...capabilities.root.beta }
      ...
      it(
        'should fail invoking a capability with expired ' +
          'second delegated capability',
        async () => {
      ...
      it(
        'should fail invoking a capability with second delegated ' +
          'capability that expires on the Unix epoch',
        async () => {
      ...
      it(
        'should fail invoking a capability with ' +
          'second delegated capability that expires after first',
        async () => {
          // bob's capability specifies a date in the future, but
      ...
      it(
        'should verify invoking a capability with only expires on ' +
          'second delegated capability',
        async () => {
      ...
      it(
        'should fail invoking a capability with only an expired ' +
          'second delegated capability',
        async () => {
    ...
    describe('Chain depth of 4', () => {
      ...
      it('should verify chain w/inspectCapabilityChain', async () => {
        ...
        const inspectCapabilityChain = async ({
          ...
          checkedChain++
          expect(Array.isArray(capabilityChain)).toBe(true)
          expect(capabilityChain).toHaveLength(4)
          expect(Array.isArray(capabilityChainMeta)).toBe(true)
          expect(capabilityChainMeta).toHaveLength(4)
          _checkCapabilityChain({ capabilityChain })
          _checkCapabilityChainMeta({ capabilityChainMeta })
          // a real implementation would look for revocations here
          return { valid: true }
      ...
      it('should fail to verify w/embedded middle zcap', async () => {
        ...
        const [error] = result.error.errors
        expect(error.message).toContain('consist of strings of capability IDs')
      })
    ...
    describe('invocationTarget attenuation', () => {
      it('should verify chain w/path attenuation', async () => {
        ...
        const invocationTarget = `${carolZcap.invocationTarget}/a-specific-document`
        const dianaZcap = await _delegate({
          newCapability: {
            '@context': ZCAP_CONTEXT_URL,
            id: uuid(),
      ...
      it('should verify chain w/query attenuation', async () => {
        ...
        const invocationTarget = `${carolZcap.invocationTarget}?foo=bar`
        const dianaZcap = await _delegate({
          newCapability: {
            '@context': ZCAP_CONTEXT_URL,
            id: uuid(),
      ...
      it('should verify chain w/path and query attenuation', async () => {
        ...
        const invocationTarget = `${carolZcap.invocationTarget}/a-specific-document?foo=bar`
        const dianaZcap = await _delegate({
          newCapability: {
            '@context': ZCAP_CONTEXT_URL,
            id: uuid(),
      ...
      it(
        'should fail to verify chain with attenuation that is more ' +
          'permissive than the parent capability w/path',
        async () => {
          const rootTarget =
          ...
          const invocationTarget = `${bobZcap.invocationTarget}/a-specific-document`
          const carolZcap = await _delegate({
            newCapability: {
              '@context': ZCAP_CONTEXT_URL,
              id: uuid(),
      ...
      it(
        'should fail to verify chain with attenuation that is more ' +
          'permissive than the parent capability w/query',
        async () => {
          const rootTarget =
          ...
          const invocationTarget = `${bobZcap.invocationTarget}?foo=bar`
          const carolZcap = await _delegate({
            newCapability: {
              '@context': ZCAP_CONTEXT_URL,
              id: uuid(),
      ...
      it(
        'should verify when invoking at an exact-match target ' + 'w/path',
        async () => {
      ...
      it(
        'should verify when invoking at an exact-match target ' + 'w/query',
        async () => {
          const rootTarget = `https://example.com/edvs/${uuid()}`
          const rootCapability = createRootCapability({
            controller: alice.id(),
      ...
      it(
        'should fail to verify when invoking at an unexpected ' +
          'target w/path',
        async () => {
          const rootTarget = `https://example.com/edvs/${uuid()}`
          const rootCapability = createRootCapability({
      ...
      it(
        ...
        async () => {
          ...
          // Note: This is intentionally an invalid target (a doc that
          // carol should not have access to)
          const invalidTarget = `${rootTarget}/a-different-specific-document?foo=bar`
          const invocation = await _invoke({
            doc,
            invoker: carol,
            purposeOptions: {
      ...
      it(
        'should verify when invoking at a valid sub target ' + 'w/path',
        async () => {
          const rootTarget = `https://example.com/edvs/${uuid()}`
          const rootCapability = createRootCapability({
            controller: alice.id(),
      ...
      it(
        'should verify when invoking at a valid sub target ' + 'w/query',
        async () => {
          const rootTarget = `https://example.com/edvs/${uuid()}`
          const rootCapability = createRootCapability({
            controller: alice.id(),
      ...
      it(
        'should fail to verify when invoking at an invalid ' + 'target w/path',
        async () => {
          const rootTarget = `https://example.com/edvs/${uuid()}`
          const rootCapability = createRootCapability({
            controller: alice.id(),
      ...
      it(
        'should fail to verify when invoking at an invalid ' + 'target w/query',
        async () => {
          const rootTarget = `https://example.com/edvs/${uuid()}`
          const rootCapability = createRootCapability({
            controller: alice.id(),
      ...
      it(
        'should fail to verify when allowTargetAttenuation is not ' +
          'explicitly allowed',
        async () => {
          const rootTarget =
          ...
          expect(result.error.errors).toHaveLength(1)
          const [error] = result.error.errors
          expect(error.message).toContain('must be equivalent to its parent')
        }
      )
      ...
      it(
        'should fail to verify an increasingly permissive chain ' +
          'w/inspectCapabilityChain',
        async () => {
          const rootTarget =
...
function _checkCapabilityChain({ capabilityChain }) {
  for (const c of capabilityChain) {
    ...
    expect(c).toHaveProperty('controller')
    expect(c).toHaveProperty('invocationTarget')
  }
}
...
function _checkCapabilityChainMeta({ capabilityChainMeta }) {
  for (const [i, m] of capabilityChainMeta.entries()) {
    ...
    expect(m).toHaveProperty('verifyResult')
    if (i === 0) {
      // root zcap verify result is `null`
      expect(m.verifyResult).toBeNull()
      continue
    }
    expect(m.verifyResult).toHaveProperty('verified')
    expect(m.verifyResult.verified).toBe(true)
    expect(m.verifyResult).toHaveProperty('results')
    expect(m.verifyResult.results.length).toBe(1)
    expect(typeof m.verifyResult.results[0]).toBe('object')
    expect(m.verifyResult.results[0]).toHaveProperty('purposeResult')
    expect(typeof m.verifyResult.results[0].purposeResult).toBe('object')
    expect(m.verifyResult.results[0].purposeResult).toHaveProperty('delegator')
  }
}

// Resolves a verification method into a signer. `key` may be a serialized
// verification method (static actors) or an already-instantiated key pair
// (did:key actors).
async function _signerFor(key) {
  const keyPair =
    typeof key?.signer === 'function'
      ? key
      : await Ed25519VerificationKey.from(key)
...
async function _invoke({
  ...
  return jsigs.sign(doc, {
    documentLoader: testLoader,
    suite: new Ed25519Signature2020({ signer: await _signerFor(key), date }),
    purpose
  })
}
